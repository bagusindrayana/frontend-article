<script>
    ...
    function fetchData() {
        fetch(
            `${PUBLIC_API_URL}/article?search=${searchTerm}&page=${curPage}&status=publish`,
        )
            .then((res) => res.json())
            .then((data) => {
                items = data.data;
                    ...
                    <TableBodyCell>
                        <div class="flex p-2 gap-2">
                            ...
                            <button
                                ...
                                type="button"
                                class="px-3 py-2 flex text-xs font-medium text-center text-white bg-red-700 rounded-lg hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800"
                            >
                                <TrashBinSolid class="w-4 h-4" /> Hapus
                            </button>
                        </div>
                    </TableBodyCell>
                </TableBodyRow>
